...
<script type="text/JavaScript">
   function PreviewImg(imgFile)
	       ...
	       document.forms[0].action="<%=path %>/bbs_forum.do?methede=updatesecondForum";
...
<input type="hidden"  name="categoryid" value="${forum.categoryid }">
  <table width="1052" border=0 align=center cellpadding=2 cellspacing=1 bordercolor="#799ae1" class=tableborder>
    <tbody>
      <tr>
        <th align=center colspan=16 style="height: 23px">修改二级论坛</th>
      </tr>
      <tr bgcolor="#dee5fa">
        <td colspan="16" align="center" class=txlrow>${msg }</td>
      </tr>
       <tr>
        <td align="right" width="30%"><b>论坛级别：</b></td><td align="left" >${forum.rank}</td>
      </tr>
       <tr>
        <td align="right" style="padding-top: 10px;"><b>所属一级论坛：</b></td>
        <td align="left">
        <select name="parentid">
         <c:forEach items="${listfirstforum}" var="fforum">
        <option <c:if test="${fforum.id eq forum.parentid}">selected="selected"</c:if> value="${fforum.id }">${fforum.forumname }</option>
        </c:forEach>
        </select>
        </td>
      </tr>
       ...
       <tr>
        <td align="right" style="padding-top: 10px;"><b>论坛说明：</b></td><td align="left" style="padding-top: 10px;"><input type="text" name="description" value=${forum.description }></td>
      </tr>
       <tr>
      ...
      <tr>
       ...
       <tr>
        ...
        <option value="正规论坛" <c:if test="${forum.ptype eq '正规论坛'}">selected="selected"</c:if>>正规论坛  【只允许注册会员发言】</option>
      ...
      <tr>
       ...
       <tr>
        ...
        <input type="button" value="提交" onclick="tj()">  <input type="button" value="返回" onclick="goto('<%=path %>/bbs_forum.do?methede=listSecondForum')">
        </td>
      </tr>
      </table>
