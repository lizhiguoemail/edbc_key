...
<html>
  <head>
    ...
    <title>${bbs_params.bbs_name }-帖子编辑</title>
  ...
  <form method="post" action="<%=path %>/bbs_post.do?methede=updatePost" enctype="multipart/form-data">
  ...
  <input type="hidden" name="posttype" value="1">
   ...
   → <a href="<%=path %>/bbs_post.do?methede=showPost&oid=${bbspost.id }" style="color: gray;">${bbspost.postname }</a>
   → 编辑帖子
   </td>
   </tr>
   <tr><td width="100%" align="center" style="padding-left: 10px;">
    ...
    <input type="button" value="修改" onclick="tj()"/> <input type="button" value="返回" onclick="window.history.go(-1)"/>  
    </td></tr>
    </table>
   </td></tr>
   </table>
