...
document.forms[0].action="<%=path %>/bbs_post.do?methede=showVote";
...
document.forms[0].submit();
    ...
    function gotovote(str)
	{
	var votevalue=document.getElementsByName("votevalue");
	var a='no';
	for(var i=0;i<votevalue.length;i++)
	 {
	    if(votevalue[i].checked)
	     {
	        a='yes';
	     }
	 }
 ...
 function votewidth(a,kuan)
  {
   var w=300*parseFloat(kuan); 
   a.style.height="10px";
   a.style.width=w+"px";
  }
...
<input  type="hidden" id="totalpage" value="${pageParm.totalpage}"/>
 ...
 <table border="1" width="100%" bordercolor="#8EBEF7">
   ...
   <td style="padding-left: 3px;padding-top: 5px;height: 100%;width: 100%;background-color: #ffffff;" valign="top">
  ...
  <input type="radio" value="${v }" name="votevalue" id="votevalue"> ${v }
   </td></tr>
   </c:forEach>
   </c:if>
    <c:if test="${empty votearr && bbs_post.islockvote ne '1' && not empty userinfo}">
   <tr><td style="border-bottom:1px dashed #ccc;width: 100%;padding-top: 5px;padding-bottom: 5px;padding-left: 10px;"><input type="button" value="参加投票" onclick="gotovote('<%=path %>/bbs_post.do?methede=vote')"></td></tr>
   </c:if>
    <c:if test="${empty votearr && bbs_post.islockvote ne '1'&& empty userinfo}">
     <tr><td style="border-bottom:1px dashed #ccc;width: 100%;padding-top: 5px;padding-bottom: 5px;padding-left: 10px;"><input type="button" value="参加投票" onclick="alert('请先登录')"></td></tr>
    </c:if>
    <c:if test="${not empty votearr || bbs_post.islockvote eq '1'}">
    <tr><td style="border-bottom:1px dashed #ccc;width: 100%;padding-top: 5px;padding-bottom: 5px;color: red;"><c:if test="${bbs_post.islockvote eq '1'}">投票已关闭</c:if><c:if test="${bbs_post.islockvote ne '1'}">您已投过票</c:if></td></tr>
   </c:if>
   <c:if test="${not empty votearr}">
    <tr><td style="padding-top: 10px;border-top:1px dashed #ccc;">投票情况：</td></tr>
    <% int j=0; %>
   <c:forEach items="${votearr}" var="arr">
   <%if(j>10){j=0;}j++; %>
   <tr><td height="20px">
   ${arr[0] }：<img src="<%=path %>/images/front/bbs/bar<%=j %>.gif" onload="votewidth(this,'${arr[2] }')" border="0">  ${arr[1] }票  <fmt:formatNumber type="percent" value="${arr[2]}"/> 
   </td></tr>
   </c:forEach>
   </c:if>
   <tr><td style="padding-top: 10px;padding-bottom: 5px;padding-left: 10px;height: 100%;">${bbs_post.postcontent }</td></tr>
