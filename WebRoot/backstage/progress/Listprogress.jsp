...
<script type="text/javascript">
  ...
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/backstage/progress/Addprogress.jsp"></iframe>' 
   });
    add.show(); 
  };  
   ...
   function shownew(id){
      ...
      html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/progress.do?methede=show&id='+id+'"></iframe>'
      });
      shownew.show();
    };
  ...
  function updatenew(id){ 
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/progress.do?methede=toUpdate&id='+id+'"></iframe>' 
     });
     update.show();
     }; 
  ...
  if (sm.hasSelection()) {
    ...
    Ext.Ajax.request({
       url: '<%=path %>/progress.do?methede=batchdeleteEXT',
       method: "post",
       params:{id:id},
  ...
  {header : '工作人',dataIndex : 'workuser',sortable:true,align:'center'}, 
  {header : '所属任务',width:180,dataIndex : 'worktaskname',sortable:true,align:'center'}, 
  {header : '工作日期',width:120,dataIndex : 'worktime',sortable:true,align:'center'}, 
  {header : '精简描述',width:200,dataIndex : 'description',sortable:true,align:'center'}, 
  ...
  {header : '录入日期',width:120,dataIndex : 'createdate',sortable:true,align:'center'}]);
   ...
   {url: '<%=path %>/progress.do?methede=listEXT',
   method : 'post'
   }),//store提交的地址 
  ...
  root: 'rows' //得到记录集合，需是json格式的，也是从后台返回的 
        ...
        {name:'id',mapping:'id'},
        {name:'worktaskname',mapping:'worktaskname'},
        {name:'worktime',mapping:'worktime'},
        {name:'workuser',mapping:'workuser'},
