...
<script type="text/javascript">
  ...
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/backstage/progress/Addworktask.jsp"></iframe>' 
   });
    add.show(); 
  };  
   ...
   function shownew(id){
      ...
      html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/worktask.do?methede=show&id='+id+'"></iframe>'
      });
      shownew.show();
    };
  ...
  function updatenew(id){ 
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/worktask.do?methede=toUpdate&id='+id+'"></iframe>' 
     });
     update.show();
     }; 
  ...
  if (sm.hasSelection()) {
    ...
    Ext.Ajax.request({
       url: '<%=path %>/worktask.do?methede=batchdeleteEXT',
       method: "post",
       params:{id:id},
  ...
  {header : '任务名称',width:200,dataIndex : 'name',sortable:true,align:'center'}, 
  ...
  {header : '创建日期',width:120,dataIndex : 'createdate',sortable:true,align:'center'}]);
   ...
   {url: '<%=path %>/worktask.do?methede=listEXT',
   method : 'post'
   }),//store提交的地址 
