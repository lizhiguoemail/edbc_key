...
<script type="text/javascript">
  ...
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%"  height="100%" src="<%=path %>/backstage/jgw/Addjgw_words.jsp"></iframe>' 
   });
    add.show(); 
  };  
  ...
  function updatenew(id){ 
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/jgw_words.do?methede=toUpdate&id='+id+'"></iframe>' 
     });
     update.show();
     }; 
  ...
  if (sm.hasSelection()) {
    ...
    Ext.Ajax.request({
       url: '<%=path %>/jgw_words.do?methede=batchdeleteEXT',
       method: "post",
       params:{id:id},
  ...
  tbar: [
   ...
   items : [  
     ...
     { text: '开始搜索',id:'searchb',iconCls:'icon_seek',handler: Search }, '-',
      ''
