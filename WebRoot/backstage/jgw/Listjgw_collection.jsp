...
<script type="text/javascript">
  ...
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%" id="addj" name="addj" height="100%" src="<%=path %>/backstage/jgw/Addjgw_collection.jsp"></iframe>' 
   });
    add.show(); 
  };  
   ...
   function shownew(id){
      ...
      html: '<iframe frameborder="0" width="100%"  id="addj" name="addj"  height="100%" src="<%=path %>/jgw_collection.do?methede=show&id='+id+'"></iframe>'
      });
      shownew.show();
    };
  ...
  function updatenew(id){ 
    ...
    html: '<iframe frameborder="0" width="100%"  id="addj" name="addj"  height="100%" src="<%=path %>/jgw_collection.do?methede=toUpdate&id='+id+'"></iframe>' 
     });
     update.show();
     }; 
  ...
  if (sm.hasSelection()) {
    ...
    Ext.Ajax.request({
       url: '<%=path %>/jgw_collection.do?methede=batchdeleteEXT',
       method: "post",
       params:{id:id},
  ...
  {header : '编号',dataIndex : 'code',sortable:true,align:'center'}, 
  {header : '所在册',dataIndex : 'book',sortable:true,align:'center'}, 
  {header : '内容',width:200,dataIndex : 'showcontent',sortable:true,align:'center'}, 
  ...
  {header : '创建日期',width:120,dataIndex : 'createdate',sortable:true,align:'center'}]);
   ...
   {url: '<%=path %>/jgw_collection.do?methede=listEXT',
   method : 'post'
   }),//store提交的地址 
  ...
  root: 'rows' //得到记录集合，需是json格式的，也是从后台返回的 
        ...
        {name:'book',mapping:'book'},
        {name:'showcontent',mapping:'showcontent'},
