...
<script type="text/javascript">
  ...
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/backstage/museum/experts/Addexperts.jsp"></iframe>' 
   });
    add.show(); 
  };  
   ...
   function shownew(id){
    ...
    height:600,
      ...
      html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/experts.do?methede=show&id='+id+'"></iframe>'
      });
      shownew.show();
    };
  ...
  function updatenew(id){ 
    ...
    listeners:{ 
     ...
     html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/experts.do?methede=toUpdate&id='+id+'"></iframe>' 
     });
     update.show();
     }; 
  ...
  if (sm.hasSelection()) {
    ...
    Ext.Ajax.request({
       url: '<%=path %>/experts.do?methede=batchdeleteEXT',
       method: "post",
       params:{id:id},
  ...
  {header : '人物名称',dataIndex : 'name',sortable:true,align:'center'}, 
  {header : '人物头像',width:155,dataIndex : 'pic',sortable:true,align:'center',renderer:function(val){return '<img width="150px" height="190px" src="<%=path %>/upload/experts/'+val+'">'}}, 
 // {header : '人物摘要',width:300,dataIndex : 'summary',sortable:true,align:'center',renderer:function(val){return '<div align="left"><pre>'+val+'</pre></div>'}}, 
  ...
  {header : '创建日期',width:120,dataIndex : 'createdate',sortable:true,align:'center'}]);
   ...
   {url: '<%=path %>/experts.do?methede=listEXT',
   method : 'post'
   }),//store提交的地址 
  ...
  tbar: [
     ...
     { text: '搜索',iconCls:'icon_seek',handler: Search }, '-',
       <c:if test="${(fn:contains(userinfo.systempermit,',1021,')==false)||((fn:contains(userinfo.systempermit,',1021,')==true)&&(fn:contains(userinfo.allpermits,',1021,')==true))||userinfo.isadmin eq '1'}">
     { text: '新建',iconCls:'icon_add', handler: addnew}, '-',
      </c:if>
       <c:if test="${(fn:contains(userinfo.systempermit,',1022,')==false)||((fn:contains(userinfo.systempermit,',1022,')==true)&&(fn:contains(userinfo.allpermits,',1022,')==true))||userinfo.isadmin eq '1'}">
     { text: '编辑',iconCls:'icon_edit', handler: Gridupdate}, '-',
      </c:if>
       <c:if test="${(fn:contains(userinfo.systempermit,',1023,')==false)||((fn:contains(userinfo.systempermit,',1023,')==true)&&(fn:contains(userinfo.allpermits,',1023,')==true))||userinfo.isadmin eq '1'}">
     { text: '删除',iconCls:'icon_delete',handler: Griddelete}, '-',
      </c:if>
       <c:if test="${(fn:contains(userinfo.systempermit,',1024,')==false)||((fn:contains(userinfo.systempermit,',1024,')==true)&&(fn:contains(userinfo.allpermits,',1024,')==true))||userinfo.isadmin eq '1'}">
     ...
     var id = record.get('id');
      <c:choose>
		   <c:when test="${(fn:contains(userinfo.systempermit,',1022,')==false)||((fn:contains(userinfo.systempermit,',1022,')==true)&&(fn:contains(userinfo.allpermits,',1022,')==true))||userinfo.isadmin eq '1'}">
		   updatenew(id);
		   </c:when>
		   <c:otherwise>
		   shownew(id);
