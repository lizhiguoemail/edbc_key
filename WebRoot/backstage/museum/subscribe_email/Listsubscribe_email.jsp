...
<script type="text/javascript">
  ...
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/backstage/museum/subscribe_email/Addsubscribe_email.jsp"></iframe>' 
   });
    add.show(); 
  };  
   ...
   function shownew(id){
      ...
      plain: false,
      resizable: true,
      html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/subscribe_email.do?methede=show&id='+id+'"></iframe>'
      });
      shownew.show();
    };
  ...
  function updatenew(id){ 
    ...
    listeners:{ 
     ...
     html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/subscribe_email.do?methede=toUpdate&id='+id+'"></iframe>' 
     });
     update.show();
     }; 
  ...
  if (sm.hasSelection()) {
    ...
    Ext.Ajax.request({
       url: '<%=path %>/subscribe_email.do?methede=batchdeleteEXT',
       method: "post",
       params:{id:id},
  ...
  {header : '邮箱地址',width:200,dataIndex : 'email',sortable:true,align:'center'}, 
  {header : 'IP地址',dataIndex : 'ip',sortable:true,align:'center'}, 
  {header : '地域',dataIndex : 'zone',sortable:true,align:'center'}, 
  {header : '详细地址',dataIndex : 'detailaddress',sortable:true,align:'center'}, 
  {header : '创建日期',width:120,dataIndex : 'createdate',sortable:true,align:'center'}]);
   cm.defaultSortable = true;
   var store = new Ext.data.Store({
   proxy: new Ext.data.HttpProxy(
   {url: '<%=path %>/subscribe_email.do?methede=listEXT',
   method : 'post'
   }),//store提交的地址 
  ...
  tbar: [
     ...
     { text: '搜索',iconCls:'icon_seek',handler: Search }, '-',
      <c:if test="${(fn:contains(userinfo.systempermit,',1031,')==false)||((fn:contains(userinfo.systempermit,',1031,')==true)&&(fn:contains(userinfo.allpermits,',1031,')==true))||userinfo.isadmin eq '1'}">
     { text: '删除',iconCls:'icon_delete',handler: Griddelete}, '-',
      </c:if>
      <c:if test="${(fn:contains(userinfo.systempermit,',1032,')==false)||((fn:contains(userinfo.systempermit,',1032,')==true)&&(fn:contains(userinfo.allpermits,',1032,')==true))||userinfo.isadmin eq '1'}">
