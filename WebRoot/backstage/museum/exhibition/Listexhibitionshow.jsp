...
<script type="text/javascript">
  ...
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/backstage/museum/exhibition/Addexhibitionshow.jsp"></iframe>' 
   });
    add.show(); 
  };  
   ...
   function shownew(id){
      ...
      html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/article.do?methede=showExhibition&id='+id+'"></iframe>'
      });
      shownew.show();
    };
  ...
  {header : '活动标题',dataIndex : 'title',sortable:true,align:'center'}, 
  {header : '开始时间',width:120,dataIndex : 'begindate',sortable:true,align:'center'}, 
  {header : '结束时间',width:120,dataIndex : 'enddate',sortable:true,align:'center'}, 
  {header : '展览状态',dataIndex : 'state',sortable:true,align:'center'}, 
  {header : '录入者',dataIndex : 'usernickname',sortable:true,align:'center'}
  ]);
   cm.defaultSortable = true;
   var store = new Ext.data.Store({
   proxy: new Ext.data.HttpProxy(
   {url: '<%=path %>/article.do?methede=listExhibitionEXT',
   method : 'post'
   }),//store提交的地址 
  ...
  tbar: [
     ...
     { text: '搜索',iconCls:'icon_seek',handler: Search }, '-',
      <c:if test="${(fn:contains(userinfo.systempermit,',1011,')==false)||((fn:contains(userinfo.systempermit,',1011,')==true)&&(fn:contains(userinfo.allpermits,',1011,')==true))||userinfo.isadmin eq '1'}">
     { text: '新建展览活动',iconCls:'icon_add', handler: addnew}, '-',
      </c:if>
      <c:if test="${(fn:contains(userinfo.systempermit,',1012,')==false)||((fn:contains(userinfo.systempermit,',1012,')==true)&&(fn:contains(userinfo.allpermits,',1012,')==true))||userinfo.isadmin eq '1'}">
     { text: '编辑',iconCls:'icon_edit', handler: Gridupdate}, '-',
      </c:if>
      <c:if test="${(fn:contains(userinfo.systempermit,',1013,')==false)||((fn:contains(userinfo.systempermit,',1013,')==true)&&(fn:contains(userinfo.allpermits,',1013,')==true))||userinfo.isadmin eq '1'}">
     ...
     var id = record.get('id');
      <c:choose>
		   <c:when test="${(fn:contains(userinfo.systempermit,',1012,')==false)||((fn:contains(userinfo.systempermit,',1012,')==true)&&(fn:contains(userinfo.allpermits,',1012,')==true))||userinfo.isadmin eq '1'}">
		   updatenew(id);
		   </c:when>
		   <c:otherwise>
		   shownew(id);
