...
<script type="text/javascript">
  ...
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/backstage/museum/exhibition/Addexhibition.jsp"></iframe>' 
   });
    add.show(); 
  };  
   ...
   function shownew(id){
      ...
      html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/exhibition.do?methede=show&id='+id+'"></iframe>'
      });
      shownew.show();
    };
  ...
  function updatenew(id){ 
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/exhibition.do?methede=toUpdate&id='+id+'"></iframe>' 
     });
     update.show();
     }; 
   ...
   if (sm.hasSelection()) { 
     ...
     Ext.Msg.alert("系统提示", "没有任何行被选中，无法进行编辑操作！");
      ...
      function setnew(id){ 
    var set = new parent.window.Ext.Window({
    id:'setnew', 
    title:'展览展品设置', 
    layout:'fit', 
    width:960,
    ...
    cls: "x-btn-text-icon", 
    handler:function(){ 
    var win = parent.Ext.getCmp('setnew'); 
    if (win) {win.close();} 
    } 
    }], 
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/exhibition.do?methede=toSet&id='+id+'"></iframe>' 
     });
     set.show();
     }; 
  ...
  if (sm.hasSelection()) {
    ...
    Ext.Ajax.request({
       url: '<%=path %>/exhibition.do?methede=batchdeleteEXT',
       method: "post",
       params:{id:id},
  ...
  {header : '展览名称',dataIndex : 'name',sortable:true,align:'center'}, 
  {header : '展览介绍',width:400,dataIndex : 'content',sortable:true,align:'center'}, 
  ...
  {header : '创建日期',width:120,dataIndex : 'createdate',sortable:true,align:'center'}]);
   ...
   {url: '<%=path %>/exhibition.do?methede=listEXT',
   method : 'post'
   }),//store提交的地址 
  ...
  tbar: [
     ...
     { text: '搜索',iconCls:'icon_seek',handler: Search }, '-',
      <c:if test="${(fn:contains(userinfo.systempermit,',1051,')==false)||((fn:contains(userinfo.systempermit,',1051,')==true)&&(fn:contains(userinfo.allpermits,',1051,')==true))||userinfo.isadmin eq '1'}">
     { text: '新建',iconCls:'icon_add', handler: addnew}, '-',
      </c:if>
       <c:if test="${(fn:contains(userinfo.systempermit,',1052,')==false)||((fn:contains(userinfo.systempermit,',1052,')==true)&&(fn:contains(userinfo.allpermits,',1052,')==true))||userinfo.isadmin eq '1'}">
     { text: '编辑',iconCls:'icon_edit', handler: Gridupdate}, '-',
      </c:if>
      <c:if test="${(fn:contains(userinfo.systempermit,',1053,')==false)||((fn:contains(userinfo.systempermit,',1053,')==true)&&(fn:contains(userinfo.allpermits,',1053,')==true))||userinfo.isadmin eq '1'}">
     { text: '展品设置',iconCls:'icon_edit', handler: Gridset}, '-',
      </c:if>
      <c:if test="${(fn:contains(userinfo.systempermit,',1054,')==false)||((fn:contains(userinfo.systempermit,',1054,')==true)&&(fn:contains(userinfo.allpermits,',1054,')==true))||userinfo.isadmin eq '1'}">
     { text: '删除',iconCls:'icon_delete',handler: Griddelete}, '-',
      </c:if>
      <c:if test="${(fn:contains(userinfo.systempermit,',1055,')==false)||((fn:contains(userinfo.systempermit,',1055,')==true)&&(fn:contains(userinfo.allpermits,',1055,')==true))||userinfo.isadmin eq '1'}">
