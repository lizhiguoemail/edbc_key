...
<html>
  <head>
      ...
      var addnew=function(){
       var add = new parent.window.Ext.Window({
	       ...
	       type: 'submit'
	        ...
	        html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/backstage/front/article/Addarticle_type.jsp"></iframe>'
	        });
            add.show();
      };
       ...
       id:'updatebutton'
        ...
        if (win) {win.close();}
	        ...
	        html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/base_type.do?methede=toUpdateBase_type&type_type=1&oid='+oid+'"></iframe>'
	        });
        update.show();
      };
       ...
       var Griddelete = function () {
           if (sm.hasSelection()) {
			       ...
			       // Ext.Ajax.defaultPostHeader = 'FTchinaMVC/json'; //仅传递ids数组即可，无需转换为json
			        Ext.Ajax.request({
			            url: '<%=path %>/base_type.do?methede=batchdeleteEXT',
			            method: "post",
			            params:{oid:id},
   ...
   {header : '文章类别名',width:300,dataIndex : 'type_name',sortable:true,align:'center'}, 
   ...
   {header : '修改日期',width:120,dataIndex : 'updatedate',sortable:true,align:'center'}]);
    cm.defaultSortable = true;
       var store = new Ext.data.Store({ 
        ...
        {url: '<%=path %>/base_type.do?methede=listBase_typeEXT',
         method : 'post'
        }),//store提交的地址 
      ...
      store.on("beforeload", function(currentStore, options) {
                ...
                currentStore.baseParams["type_type"] = 1;
            });
    ...
    var grid = new Ext.grid.GridPanel({ 
        ...
        { text: '搜索',iconCls:'icon_seek',handler: Search }, '-',
         <c:if test="${(fn:contains(userinfo.systempermit,',731,')==false)||((fn:contains(userinfo.systempermit,',731,')==true)&&(fn:contains(userinfo.allpermits,',731,')==true))||userinfo.isadmin eq '1'}">
        { text: '新建',iconCls:'icon_add', handler: addnew}, '-',
         </c:if>
         <c:if test="${(fn:contains(userinfo.systempermit,',732,')==false)||((fn:contains(userinfo.systempermit,',732,')==true)&&(fn:contains(userinfo.allpermits,',732,')==true))||userinfo.isadmin eq '1'}">
        { text: '编辑',iconCls:'icon_edit', handler: Gridupdate}, '-',
         </c:if>
         <c:if test="${(fn:contains(userinfo.systempermit,',733,')==false)||((fn:contains(userinfo.systempermit,',733,')==true)&&(fn:contains(userinfo.allpermits,',733,')==true))||userinfo.isadmin eq '1'}">
         ...
         listeners:{
           "rowdblclick" : function(grid, rowIndex, e){
            ...
            var id = record.get('id');
             <c:choose>
		   <c:when test="${(fn:contains(userinfo.systempermit,',732,')==false)||((fn:contains(userinfo.systempermit,',732,')==true)&&(fn:contains(userinfo.allpermits,',732,')==true))||userinfo.isadmin eq '1'}">
		   updatenew(id);
		   </c:when>
		   <c:otherwise>
		   shownew(companyid);
