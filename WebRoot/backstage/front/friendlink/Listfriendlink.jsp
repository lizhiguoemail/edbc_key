...
<script type="text/javascript">
  ...
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/backstage/front/friendlink/Addfriendlink.jsp"></iframe>' 
   });
    add.show(); 
  };  
   ...
   function shownew(id){
    ...
    height:200,
      ...
      html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/friendlink.do?methede=show&id='+id+'"></iframe>'
      });
      shownew.show();
    };
  ...
  function updatenew(id){ 
    ...
    listeners:{ 
     ...
     html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/friendlink.do?methede=toUpdate&id='+id+'"></iframe>' 
     });
     update.show();
     }; 
  ...
  if (sm.hasSelection()) {
    ...
    Ext.Ajax.request({
       url: '<%=path %>/friendlink.do?methede=batchdeleteEXT',
       method: "post",
       params:{id:id},
  ...
  {header : '链接名称',dataIndex : 'name',sortable:true,align:'center'}, 
  {header : '链接地址',width:250,dataIndex : 'linkurl',sortable:true,align:'left'}, 
  {header : '排序码',width:60,dataIndex : 'sortcode',sortable:true,align:'center',type:'double'},
  {header : '创建日期',width:150,dataIndex : 'createdate',sortable:true,align:'center'},
  {header : '修改日期',width:150,dataIndex : 'updatedate',sortable:true,align:'center'}]);
   cm.defaultSortable = true;
   var store = new Ext.data.Store({
   proxy: new Ext.data.HttpProxy(
   {url: '<%=path %>/friendlink.do?methede=listEXT',
   method : 'post'
   }),//store提交的地址 
  ...
  tbar: [
     ...
     { text: '搜索',iconCls:'icon_seek',handler: Search }, '-',
      <c:if test="${(fn:contains(userinfo.systempermit,',751,')==false)||((fn:contains(userinfo.systempermit,',751,')==true)&&(fn:contains(userinfo.allpermits,',751,')==true))||userinfo.isadmin eq '1'}">
     { text: '新建',iconCls:'icon_add', handler: addnew}, '-',
     </c:if>
      <c:if test="${(fn:contains(userinfo.systempermit,',752,')==false)||((fn:contains(userinfo.systempermit,',752,')==true)&&(fn:contains(userinfo.allpermits,',752,')==true))||userinfo.isadmin eq '1'}">
     { text: '编辑',iconCls:'icon_edit', handler: Gridupdate}, '-',
     </c:if>
      <c:if test="${(fn:contains(userinfo.systempermit,',753,')==false)||((fn:contains(userinfo.systempermit,',753,')==true)&&(fn:contains(userinfo.allpermits,',753,')==true))||userinfo.isadmin eq '1'}">
     { text: '删除',iconCls:'icon_delete',handler: Griddelete}, '-',
     </c:if>
      <c:if test="${(fn:contains(userinfo.systempermit,',754,')==false)||((fn:contains(userinfo.systempermit,',754,')==true)&&(fn:contains(userinfo.allpermits,',754,')==true))||userinfo.isadmin eq '1'}">
     ...
     <c:choose>
		   <c:when test="${(fn:contains(userinfo.systempermit,',752,')==false)||((fn:contains(userinfo.systempermit,',752,')==true)&&(fn:contains(userinfo.allpermits,',752,')==true))||userinfo.isadmin eq '1'}">
		   updatenew(id);
		   </c:when>
		   <c:otherwise>
		   shownew(id);
