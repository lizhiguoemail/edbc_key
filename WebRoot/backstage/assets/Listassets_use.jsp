...
<script type="text/javascript">
  ...
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/backstage/assets/Addassets_use.jsp"></iframe>' 
   });
    add.show(); 
  };  
   ...
   function shownew(id){
      ...
      plain: false,
      resizable: true,
      html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/assets_use.do?methede=show&id='+id+'"></iframe>'
      });
      shownew.show();
    };
  ...
  function updatenew(id){ 
    ...
    listeners:{ 
     ...
     html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/assets_use.do?methede=toUpdate&id='+id+'"></iframe>' 
     });
     update.show();
     }; 
  ...
  if (sm.hasSelection()) {
    ...
    Ext.Ajax.request({
       url: '<%=path %>/assets_use.do?methede=batchdeleteEXT',
       method: "post",
       params:{id:id},
  ...
  {header : '使用者',dataIndex : 'username',sortable:true,align:'center'}, 
  {header : '预计归还时间',dataIndex : 'ruturntime',sortable:true,align:'center'}, 
  ...
  {header : '操作日期',width:120,dataIndex : 'createdate',sortable:true,align:'center'}]);
   ...
   {url: '<%=path %>/assets_use.do?methede=listEXT',
   method : 'post'
   }),//store提交的地址 
  ...
  root: 'rows' //得到记录集合，需是json格式的，也是从后台返回的 
        ...
        {name:'ruturntime',mapping:'ruturntime'},
  ...
  tbar: [
     ...
     { text: '搜索',iconCls:'icon_seek',handler: Search }, '-',
      <c:if test="${(fn:contains(userinfo.systempermit,',921,')==false)||((fn:contains(userinfo.systempermit,',921,')==true)&&(fn:contains(userinfo.allpermits,',921,')==true))||userinfo.isadmin eq '1'}">
