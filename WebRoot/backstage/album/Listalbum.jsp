...
<script type="text/javascript">
  ...
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/backstage/album/Addalbum.jsp"></iframe>' 
   });
    add.show(); 
  };  
   ...
   function shownew(id){
      ...
      html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/album.do?methede=show&id='+id+'"></iframe>'
      });
      shownew.show();
    };
  ...
  function updatenew(id){ 
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/album.do?methede=toUpdate&id='+id+'"></iframe>' 
     });
     update.show();
     }; 
  ...
  if (sm.hasSelection()) {
    ...
    Ext.Ajax.request({
       url: '<%=path %>/album.do?methede=batchdeleteEXT',
       method: "post",
       params:{id:id},
  ...
  {header : '标题',dataIndex : 'name',sortable:true,align:'center'}, 
  {header : '类别',dataIndex : 'category',sortable:true,align:'center'}, 
  {header : '创建日期',width:120,dataIndex : 'createdate',sortable:true,align:'center'}]);
   cm.defaultSortable = true;
   var store = new Ext.data.GroupingStore({
   proxy: new Ext.data.HttpProxy(
   {url: '<%=path %>/album.do?methede=listEXT',
   method : 'post'
   }),//store提交的地址 
