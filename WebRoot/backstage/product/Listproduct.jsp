...
<script type="text/javascript">
  ...
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/backstage/product/Addproduct.jsp"></iframe>' 
   });
    add.show(); 
  };  
   ...
   function shownew(id){
      ...
      html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/product.do?methede=show&id='+id+'"></iframe>'
      });
      shownew.show();
    };
  ...
  function updatenew(id){ 
    ...
    listeners:{ 
     ...
     html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/product.do?methede=toUpdate&id='+id+'"></iframe>' 
     });
     update.show();
     }; 
  ...
  if (sm.hasSelection()) {
    ...
    Ext.Ajax.request({
       url: '<%=path %>/product.do?methede=batchdeleteEXT',
       method: "post",
       params:{id:id},
  ...
  {header : '产品编号',dataIndex : 'code',sortable:true,align:'center'}, 
  {header : '产品名称',dataIndex : 'name',sortable:true,align:'center'}, 
  {header : '单位',dataIndex : 'unit',sortable:true,align:'center'}, 
  {header : '上限数量',dataIndex : 'cap',sortable:true,align:'center',type:'int'}, 
  {header : '下限数量',dataIndex : 'lower',sortable:true,align:'center',type:'int'}, 
  ...
  {header : '修改日期',width:120,dataIndex : 'updatedate',sortable:true,align:'center'}]);
   ...
   {url: '<%=path %>/product.do?methede=listEXT',
   method : 'post'
   }),//store提交的地址 
  ...
  root: 'rows' //得到记录集合，需是json格式的，也是从后台返回的 
        ...
        {name:'cap',mapping:'cap'},
        {name:'lower',mapping:'lower'},
  ...
  tbar: [
     ...
     { text: '搜索',iconCls:'icon_seek',handler: Search }, '-',
       <c:if test="${(fn:contains(userinfo.systempermit,',811,')==false)||((fn:contains(userinfo.systempermit,',811,')==true)&&(fn:contains(userinfo.allpermits,',811,')==true))||userinfo.isadmin eq '1'}">
     { text: '产品登记',iconCls:'icon_add', handler: addnew}, '-',
      </c:if>
       <c:if test="${(fn:contains(userinfo.systempermit,',812,')==false)||((fn:contains(userinfo.systempermit,',812,')==true)&&(fn:contains(userinfo.allpermits,',812,')==true))||userinfo.isadmin eq '1'}">
     { text: '产品编辑',iconCls:'icon_edit', handler: Gridupdate}, '-',
      </c:if>
       <c:if test="${(fn:contains(userinfo.systempermit,',813,')==false)||((fn:contains(userinfo.systempermit,',813,')==true)&&(fn:contains(userinfo.allpermits,',813,')==true))||userinfo.isadmin eq '1'}">
     { text: '删除',iconCls:'icon_delete',handler: Griddelete}, '-',
      </c:if>
       <c:if test="${(fn:contains(userinfo.systempermit,',814,')==false)||((fn:contains(userinfo.systempermit,',814,')==true)&&(fn:contains(userinfo.allpermits,',814,')==true))||userinfo.isadmin eq '1'}">
