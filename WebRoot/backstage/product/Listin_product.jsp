...
<script type="text/javascript">
  ...
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/backstage/product/Addin_product.jsp"></iframe>' 
   });
    add.show(); 
  };  
   ...
   function shownew(id){
      ...
      html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/in_product.do?methede=show&id='+id+'"></iframe>'
      });
      shownew.show();
    };
  ...
  function updatenew(id){ 
    ...
    listeners:{ 
     ...
     html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/in_product.do?methede=toUpdate&id='+id+'"></iframe>' 
     });
     update.show();
     }; 
  ...
  if (sm.hasSelection()) {
    ...
    Ext.Ajax.request({
       url: '<%=path %>/in_product.do?methede=batchdeleteEXT',
       method: "post",
       params:{id:id},
  ...
  {header : '入库日期',width:120,dataIndex : 'createdate',sortable:true,align:'center'},
  ...
  tbar: [
     ...
     { text: '搜索',iconCls:'icon_seek',handler: Search }, '-',
       <c:if test="${(fn:contains(userinfo.systempermit,',821,')==false)||((fn:contains(userinfo.systempermit,',821,')==true)&&(fn:contains(userinfo.allpermits,',821,')==true))||userinfo.isadmin eq '1'}">
     { text: '产品入库',iconCls:'icon_add', handler: addnew}, '-',
      </c:if>
       <c:if test="${(fn:contains(userinfo.systempermit,',822,')==false)||((fn:contains(userinfo.systempermit,',822,')==true)&&(fn:contains(userinfo.allpermits,',822,')==true))||userinfo.isadmin eq '1'}">
     { text: '入库编辑',iconCls:'icon_edit', handler: Gridupdate}, '-',
      </c:if>
       <c:if test="${(fn:contains(userinfo.systempermit,',823,')==false)||((fn:contains(userinfo.systempermit,',823,')==true)&&(fn:contains(userinfo.allpermits,',823,')==true))||userinfo.isadmin eq '1'}">
     { text: '删除',iconCls:'icon_delete',handler: Griddelete}, '-',
      </c:if>
       <c:if test="${(fn:contains(userinfo.systempermit,',824,')==false)||((fn:contains(userinfo.systempermit,',824,')==true)&&(fn:contains(userinfo.allpermits,',824,')==true))||userinfo.isadmin eq '1'}">
    ...
    <c:choose>
		   <c:when test="${(fn:contains(userinfo.systempermit,',822,')==false)||((fn:contains(userinfo.systempermit,',822,')==true)&&(fn:contains(userinfo.allpermits,',822,')==true))||userinfo.isadmin eq '1'}">
		   updatenew(id);
		   </c:when>
		   <c:otherwise>
		   shownew(id);
