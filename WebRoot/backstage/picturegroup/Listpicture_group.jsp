...
<script type="text/javascript">
  ...
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/backstage/picturegroup/Addpicture_group.jsp"></iframe>' 
   });
    add.show(); 
  };  
   ...
   function shownew(id){
      ...
      html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/picture_group.do?methede=show&id='+id+'"></iframe>'
      });
      shownew.show();
    };
  ...
  function updatenew(id){ 
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/picture_group.do?methede=toUpdate&id='+id+'"></iframe>' 
     });
     update.show();
     }; 
  ...
  if (sm.hasSelection()) {
    ...
    Ext.Ajax.request({
       url: '<%=path %>/picture_group.do?methede=batchdeleteEXT',
       method: "post",
       params:{id:id},
  ...
  {header : '名称',dataIndex : 'name',sortable:true,align:'center'}, 
  {header : '类别',dataIndex : 'grouptype',sortable:true,align:'center',renderer:function(val){if(val==1) return '人文'; if(val==2) return '飞机'}}, 
  {header : '可见性',dataIndex : 'visibility',sortable:true,align:'center',renderer:function(val){if(val==1) return '只有自己可见'}}, 
  {header : '描述',width:120,dataIndex : 'description',sortable:true,align:'center'},
