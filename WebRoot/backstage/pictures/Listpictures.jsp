...
//下拉框
  ...
  var gid=Ext.getCmp("content2").value;
  if(gid==''||gid==null){
  Ext.Msg.alert('提示','请选择图册');
  return;
  }
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/pictures.do?methede=toaddpiliang&groupid='+gid+'"></iframe>' 
   });
    add.show(); 
  };  
  var addnew=function(){
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/backstage/pictures/Addpictures.jsp"></iframe>' 
   });
    add.show(); 
  };  
   ...
   function shownew(id){
      ...
      html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/pictures.do?methede=show&id='+id+'"></iframe>'
      });
      shownew.show();
    };
  ...
  function updatenew(id){ 
    ...
    html: '<iframe frameborder="0" width="100%" height="100%" src="<%=path %>/pictures.do?methede=toUpdate&id='+id+'"></iframe>' 
     });
     update.show();
     }; 
  ...
  if (sm.hasSelection()) {
    ...
    Ext.Ajax.request({
       url: '<%=path %>/pictures.do?methede=batchdeleteEXT',
       method: "post",
       params:{id:id},
  ...
  {header : '所在图册',dataIndex : 'picture_groupname',sortable:true,align:'center'}, 
  {header : '上传名称',dataIndex : 'oldname',sortable:true,align:'center'},
  {header : '图片',dataIndex : 'path',align:'center',renderer:function(val){
  return '<img style="width:50px;height:50px;" src="<%=path %>/'+val+'" />';
  }}, 
  {header : '类型',dataIndex : 'pictype',sortable:true,align:'center'}, 
  ...
  {header : '创建日期',width:120,dataIndex : 'createdate',sortable:true,align:'center'}]);
   ...
   proxy: new Ext.data.HttpProxy(
   {url: '<%=path %>/pictures.do?methede=listEXT',
   method : 'post'
   }),//store提交的地址 
  reader: new Ext.data.JsonReader({ 
  ...
  root: 'rows' //得到记录集合，需是json格式的，也是从后台返回的 
        ...
        {name:'picture_groupname',mapping:'picture_groupname'},
        {name:'oldname',mapping:'oldname'},
        {name:'localtionname',mapping:'localtionname'},
        {name:'pictype',mapping:'pictype'},
  ...
  store.on("beforeload", function(currentStore, options) {
   ...
   currentStore.baseParams["groupid"] = Ext.getCmp("content2").value;
   });
