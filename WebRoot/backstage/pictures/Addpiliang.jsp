...
<script type="text/javascript" src="<%=path %>/extjs/ext-all.js"></script>
  <script type="text/javascript">
			...
			window.onload = function () {
				swfu = new SWFUpload({
					upload_url: "<%=path %>/pictures.do?methede=upload&userid=${userinfo.id}&groupid=${groupid}",
					// File Upload Settings
					file_size_limit : "1000MB",	// 1000MB
					file_types : "*.jpg;*png;*.gif;*.jpeg;*.bmp",//设置可上传的类型
					...
					// Button Settings
					button_image_url : "<%=path%>/images/backstage/SmallSpyGlassWithTransperancy_17x18.png",
  ...
  Ext.form.Field.prototype.msgTarget = 'side';
   ...
   <form action="" method="post" id="form1" name="form1" >
   	<table width="100%" border="0" cellspacing="0" cellpadding="10" style="margin-top: 8px; color: #5d3b15" class="small2">
			...
			<option value="${g.id }" <c:if test="${groupid eq g.id}">selected="selected"</c:if>>${g.name}</option>
			</c:forEach>
			</select>
			</td>
		</tr>
