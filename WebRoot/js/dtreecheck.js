...
dTree.prototype.node = function(node, nodeId) {
	...
	if (node.url) {
		...
		str += '<a href="javascript: ' + this.obj + '.o(' + nodeId + ');" class="node" alt="'+node.id+'">';
	str += node.name;		if (node.url || ((!this.config.folderLinks || !node.url) && node._hc)) str += '</a>';   if(this.config.check==true){    str+= '<input type="checkbox" id="c'+ this.obj + nodeId + '" name="permits" value="'+node.id+'" alt="'+nodeId+'" onclick="javascript:'+this.obj+'.cc('+nodeId+')"/>';   }
	str += '</div>';
	if (node._hc) {
...
dTree.prototype.indent = function(node, nodeId) {
	...
	if (this.root.id != node.pid) {
		...
		if (node._hc) {
			str += '<a href="javascript: ' + this.obj + '.o(' + nodeId + ');" alt="'+node.id+'"><img id="j' + this.obj + nodeId + '" src="';
